...
ring effects. The `outer_ring` border helper is also re-exported
for use by other rect-shaped renderers (image, component) so the
"halo" border is consistently available across the shape surface.
...
def outer_ring_rect_svg(
    ...
    *,
    ...
    """Build an outer-ring `<rect>` element for a rect-shaped object.

    Returns None when `ring` is falsy. Mirrors the geometry maths used
    inline by `render_rect`: the ring is expanded outward by
    `gap + width / 2` so its inner edge sits `gap` px away from the
    shape's edge, and corner radius grows to follow rounded shapes.

    `gap` and `offset` are accepted as synonyms (`gap` matches the
    rect schema; `offset` matches the ellipse schema in this module).
    """
    if not ring:
        return None
    rc = r.color(ring.get("color"), "#000000")
    rw = fnum(ring.get("width"), 2)
    # Accept `gap` (rect convention) or `offset` (ellipse convention).
    gap = fnum(ring.get("gap", ring.get("offset")), 4)
    expand = gap + rw / 2
...
def render_rect(r: RendererContext, obj: Mapping[str, Any]) -> str:
    ...
    # Shares the helper with image/component so border behaviour is
    # consistent across rect-shaped renderers.
