...
| **4** | **Sitemap Emission — `emit_sitemap(fs, base_url, target_filter=…)`; `list_frameset_target_union`; `framegraph sitemap <input.yml> --base-url <url> [-o <path>] [--target <name>]`; one URL per (Frame × declared target); URL-escaping; sitemap.org 0.9 schema; works on any input via `coerce_to_frameset`** | **XS** | ✅ Shipped (commit `af932ac`) |
| **5** | **Per-target Adjustments — `FrameTargetAdjustments` typed model (font_scale, hide, padding_delta); `apply_target_adjustments(doc, adj)` mutates the projected single-doc dict in fixed order; wired into `build_frame_doc` so `render_frameset` picks them up automatically. Application: font_scale walks `visual.tokens.text_styles[*].size`; hide drops layers and top-level objects by id; padding_delta insets `scene.canvas.size` by `2·delta` per axis (pattern margins scale proportionally because they derive from canvas size)** | **M** | ✅ Shipped (commit `8446378`) |
| **6** | **Link Injection — `inject_svg_navigation_links(svg, frame, frameset, *, target_name, base_url=None, file_template=None)` wraps the rendered SVG body in `<a href>` per `frame.next` (click-anywhere-to-advance — the canonical deck nav pattern, survives SVG → PDF-vector and SVG → HTML-embed). URL strategies: sitemap-style `<base_url>/<target_name>/<frame_id>` (Phase 4 compatible) or Python `str.format` template like `"slide_{frame_id}.svg"`. `<title>`/`<desc>` stay outside the link for accessibility. CLI: `framegraph render --link-base-url`/`--link-template`, `framegraph deck --link-base-url`/`--link-template` (deck post-processes every per-slide SVG)** | **S–M** | ✅ Shipped (this commit) |
