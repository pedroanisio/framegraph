...
class TestComposeDocument:
    def test_compose_returns_validated_document(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
    ...
    def test_compose_emits_one_object_per_zone(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
        doc = compose_document(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
    ...
    def test_compose_assigns_box_to_each_object(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
        doc = compose_document(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
...
class TestEndToEndRender:
    def test_render_pattern_svg_returns_valid_svg(
        ...
        svg = render_pattern_svg(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
    ...
    def test_rendered_svg_contains_fill_text(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
    ...
    def test_render_is_deterministic(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
...
class TestGoldenSnapshot:
    ...
    def test_bmc_golden_snapshot_matches(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
        svg = render_pattern_svg(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
        if not BMC_GOLDEN.exists():
            BMC_GOLDEN.parent.mkdir(parents=True, exist_ok=True)
            BMC_GOLDEN.write_text(svg, encoding="utf-8")
            pytest.skip(f"captured golden at {BMC_GOLDEN}; rerun the test to verify.")
...
class TestListObjectTableEmission:
    ...
    def test_bmc_revenue_streams_emits_table(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
        doc = compose_document(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
        objects = [o for layer in doc["visual"]["layers"] for o in layer["objects"]]
        revenue_obj = next((o for o in objects if o.get("id") == "zone_revenue_streams"), None)
        assert revenue_obj is not None
        assert revenue_obj["type"] == "table", (
            f"revenue_streams should emit a table; got {revenue_obj['type']!r}"
        )

    def test_bmc_cost_structure_emits_table(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
        doc = compose_document(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
        objects = [o for layer in doc["visual"]["layers"] for o in layer["objects"]]
        cost_obj = next((o for o in objects if o.get("id") == "zone_cost_structure"), None)
    ...
    def test_bmc_table_rows_carry_field_values(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
    ...
    def test_bmc_other_zones_still_bullet_list(self, bmc_pattern, bmc_filled, bmc_layout) -> None:
        """Non-object list_items zones (the 7 simple BMC blocks) still
        render as bullet_list — the override is per-zone."""
        doc = compose_document(bmc_pattern, bmc_filled, bmc_layout, CANVAS_W, CANVAS_H)
        objects = [o for layer in doc["visual"]["layers"] for o in layer["objects"]]
        key_partners_obj = next((o for o in objects if o.get("id") == "zone_key_partners"), None)
...
class TestNegative:
    def test_wrong_content_shape_rejected(self, bmc_pattern, bmc_sidecar, bmc_layout) -> None:
    ...
    def test_compose_with_missing_role_box_raises(self, bmc_pattern, bmc_filled) -> None:
        ...
        with pytest.raises(KeyError, match="value_propositions"):
            compose_document(bmc_pattern, bmc_filled, broken_layout, CANVAS_W, CANVAS_H)
