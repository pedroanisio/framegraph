...
**framegraph** is a YAML-first DSL and pure-Python renderer for generating
diagram- and slide-grade SVG from versioned source files. The codebase is
organized around a stable rendering core, renderer modules by object family,
and regression fixtures that define acceptable output drift.

```
.
├── CLAUDE.md                # This file — project guidelines for AI agents
├── PURPOSE.md               # Mission, audience, and non-goals
├── DISCLAIMER.md            # Methodological caveats (all READMEs must reference)
├── README.md                # User-facing package overview
├── pyproject.toml           # Packaging, toolchain, and lint/test config
├── framegraph/              # Renderer, CLI, library, built-in assets
├── tests/                   # Unit, integration, fixtures, and golden tests
└── docs/                    # Active design notes plus archived planning docs
```
